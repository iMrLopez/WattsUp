...
def main():
    parser = argparse.ArgumentParser(
        description="Medición de métricas del sistema WattsUp"
    )
    parser.add_argument("--endpoint", required=True, help="URL del endpoint /estimate")
    parser.add_argument(
        "--payloads", required=True, help="Archivo JSON con lista de payloads"
    )
    parser.add_argument(
        "--cpu-power", type=float, required=True, help="Potencia estimada CPU (W)"
    )
    parser.add_argument(
        "--ram-power", type=float, required=True, help="Potencia estimada RAM (W)"
    )
    parser.add_argument(
        "--classification-csv", help="CSV para evaluar calidad de inferencia"
    )
    parser.add_argument(
        "--classification-endpoint", help="Endpoint para predicción de clasificación"
    )
    ...
    print("\n=== Medición Energética ===")
    energy_stats = measure_energy(inf_stats["tiempos"], args.cpu_power, args.ram_power)
